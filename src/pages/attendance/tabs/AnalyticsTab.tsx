...
export default function AnalyticsTab({ dropdowns }: { dropdowns?: any }) {
    ...
    dispatch({
      ...
      getResponse: (res: any) => {
        if (res?.success && res?.data) dispatch(setAnalytics(res.data));
          ...
          dispatch(setAttendanceError("Failed to load analytics"));
          toast.error("Failed to load analytics.");
        }
      },
      getError: (err: any) => {
        const msg = err?.response?.data?.message || err?.message || "Failed to fetch analytics";
        dispatch(setAttendanceError(msg));
        toast.error(msg);
      },
    });
  };

  const fetchBatchWise = () => {
    const p = new URLSearchParams();
    if (filters.date_from) p.set("date_from", filters.date_from);
    if (filters.date_to) p.set("date_to", filters.date_to);
    if (filters.branch_id) p.set("branch", filters.branch_id);

    dispatch({
      type: "GET_DROPDOWN", // Use a generic dispatch to just fetch and callback
      method: "GET",
      endPoint: `/api/v1/attendance/batch-wise/${p.toString() ? `?${p}` : ""}`,
      auth: true,
      getResponse: (res: any) => {
        if (res?.success && res?.data) setBatchWiseData(res.data);
        else setBatchWiseData([]);
      },
    } as any);
  };
  ...
  if (analyticsLoading && !analytics)
    return <TableSkeleton columns={3} rows={4} className="mt-0" />;
  ...
  const trendData = analytics?.attendance_trends
    ? filters.trend_type === "daily"
      ...
      : filters.trend_type === "weekly"
        ? analytics.attendance_trends.weekly_trend?.map((w) => ({
            date: w.week_start_date,
            percentage: w.percentage,
          }))
        : analytics.attendance_trends.monthly_trend?.map((m) => ({
            date: m.month,
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-border p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <Select
                ...
                onValueChange={(v) => setFilters((f) => ({ ...f, trend_type: v }))}
              >
          ...
          {analytics.branch_comparison?.length > 0 && (
            <div className="bg-white rounded-xl border border-border p-5">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart
                  data={analytics.branch_comparison}
          ...
          {batchWiseData?.length > 0 && (
            <div className="bg-white rounded-xl border border-border p-5">
                ...
                <BarChart data={batchWiseData} margin={{ top: 4, right: 16, left: 0, bottom: 40 }}>
                  ...
                  <Tooltip
                    formatter={(v: number, name: string) => [
                      name === "attendance_percentage" ? `${v.toFixed(1)}%` : v,
                  ...
                  />
                  <Bar dataKey="attendance_percentage" radius={[6, 6, 0, 0]} fill="#3B82F6">
                    {batchWiseData.map((_, idx) => (
                      <Cell key={idx} fill={CHART_COLORS[idx % CHART_COLORS.length]} />
                    ))}
                  </Bar>
                </BarChart>
              ...
              {/* Optional: Batch Data Table below the chart */}
                ...
                <table className="w-full text-xs text-left">
                  <thead className="text-muted-foreground border-b border-border">
                    <tr>
                      ...
                      <th className="py-2 font-medium text-red-600">Absent</th>
                      <th className="py-2 font-medium">Attendance %</th>
                    </tr>
                  </thead>
                  <tbody>
                    {batchWiseData.map((b) => (
                      <tr key={b.batch_id} className="border-b border-border/40">
                        <td className="py-2 font-medium text-foreground">
                          {b.batch_name} ({b.batch_code})
                        </td>
                        <td className="py-2">{b.total_students}</td>
                        <td className="py-2 text-emerald-600 font-medium">{b.present_count}</td>
                        <td className="py-2 text-red-600 font-medium">{b.absent_count}</td>
                        <td className="py-2 font-medium">{b.attendance_percentage}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
