...
function LeaveApplicationDetail({ item }: { item: LeaveApplication }) {
  const isStudentLeave = STUDENT_LEAVE_TYPE_OPTS.some(o => o.value === item.leave_type);
  return (
    <div className="space-y-4 mt-4">
      ...
      <div className="border border-border rounded-lg p-3 space-y-2">
        ...
        {isStudentLeave ? (
          <div className="flex items-center gap-2 text-sm">
            ...
            {item.reviewed_at && (
              <span className="text-xs text-muted-foreground ml-auto">{new Date(item.reviewed_at).toLocaleString()}</span>
            )}
          </div>
        ) : (
          <>
      ...
      {(item.supporting_document_url || item.proof_document_url) && (
        <a href={(item.supporting_document_url || item.proof_document_url) as string} target="_blank" rel="noreferrer"
...
export default function ApplicationsTab() {
  ...
  const handleRowClick = (app: LeaveApplication) => {
    ...
    const isStudentLeave = role === "student" || role === "parents" || STUDENT_LEAVE_TYPE_OPTS.some(o => o.value === app.leave_type);
    dispatch({
  ...
  useEffect(() => { fetchApplications(); }, [statusFilter, typeFilter, activeTab]);
  ...
  const handleApply = () => {
    ...
    if (role === "student" || role === "parents") {
      ...
      if (applyForm.supporting_document) formData.append("proof_document", applyForm.supporting_document);
    } else {
  ...
  const handleEdit = () => {
    ...
    const isStudentLeave = role === "student" || role === "parents" || STUDENT_LEAVE_TYPE_OPTS.some(o => o.value === editTarget.leave_type);
    dispatch({
      type: leaveActions.UPDATE_LEAVE,
      method: "PATCH",
      endPoint: isStudentLeave ? API.LEAVE.STUDENT_DETAIL(editTarget.id) : API.LEAVE.DETAIL(editTarget.id),
  ...
  const handleCancel = () => {
    ...
    const isStudentLeave = role === "student" || role === "parents" || STUDENT_LEAVE_TYPE_OPTS.some(o => o.value === cancelTarget.leave_type);
    dispatch({
      type: leaveActions.CANCEL_LEAVE,
      method: "DELETE",
      endPoint: isStudentLeave ? API.LEAVE.STUDENT_DETAIL(cancelTarget.id) : API.LEAVE.DETAIL(cancelTarget.id),
      auth: true,
  ...
  const handleApprove = () => {
    ...
    const isStudentLeave = role === "student" || role === "parents" || STUDENT_LEAVE_TYPE_OPTS.some(o => o.value === approveTarget.leave_type);
    dispatch({
      type: leaveActions.APPROVE_LEAVE,
      method: "POST",
      endPoint: isStudentLeave ? API.LEAVE.STUDENT_APPROVE(approveTarget.id) : API.LEAVE.APPROVE(approveTarget.id),
      auth: true,
  ...
  const handleReject = () => {
    ...
    const isStudentLeave = role === "student" || role === "parents" || STUDENT_LEAVE_TYPE_OPTS.some(o => o.value === rejectTarget.leave_type);
    dispatch({
  ...
  const filtered = applications.filter(a => {
    ...
    return (a.applied_by_name || a.student_name)?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="space-y-4">
          ...
          {role !== "student" && role !== "parents" && (
            <Select value={typeFilter} onValueChange={v => setTypeFilter(v === "all" ? "" : v)}>
              ...
              <SelectContent>
                ...
                {(activeTab === "student" ? STUDENT_LEAVE_TYPE_OPTS : LEAVE_TYPE_OPTS).map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
              </SelectContent>
            </Select>
          )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                >
                  <td className="px-4 py-3 font-medium text-sm">{app.applied_by_name || app.student_name}</td>
                  ...
                  <td className="px-4 py-3">
                    {STUDENT_LEAVE_TYPE_OPTS.some(o => o.value === app.leave_type) ? (
                      app.status === "approved"
                        ? <span className="text-green-600 text-xs flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Done</span>
                        : <span className="text-yellow-600 text-xs flex items-center gap-1"><Clock className="w-3 h-3" /> {app.status === "pending" ? "Pending" : app.status_display}</span>
                    ) : (
                  ...
                  <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center gap-1">
                      {canApprove && (app.status === "approval_pending" || app.status === "pending") && (
                      ...
                      {!isAdmin && (app.status === "approval_pending" || app.status === "pending") && (
                      ...
                      {(app.supporting_document_url || app.proof_document_url) && (
                        <a href={(app.supporting_document_url || app.proof_document_url) as string} target="_blank" rel="noreferrer">
      ...
      <Dialog open={applyOpen} onOpenChange={setApplyOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3 py-2">
                ...
                {applyForm.from_date && applyForm.to_date && applyForm.from_date === applyForm.to_date && (
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input type="time" value={applyForm.from_time} onChange={e => setApplyForm(f => ({ ...f, from_time: e.target.value }))} className="h-9 text-sm" />
                    </div>
                    <div>
                      <Label className="text-xs mb-1 block">To Time</Label>
                      <Input type="time" value={applyForm.to_time} onChange={e => setApplyForm(f => ({ ...f, to_time: e.target.value }))} className="h-9 text-sm" />
                    </div>
                  </div>
                )}
            ...
            {(applyForm.leave_type === "sick" || applyForm.leave_type === "medical" || role === "student" || role === "parents" || applyForm.is_capable_of_proof) && (
              <div>
                <Label className="text-xs mb-1 block">{(role === "student" || role === "parents") ? "Proof Document" : "Supporting Document (required if > 2 days)"}</Label>
      ...
      <ConfirmDialog
        ...
        description={STUDENT_LEAVE_TYPE_OPTS.some(o => o.value === approveTarget?.leave_type) ? "Approve this student leave request." : `Step ${role === "admin_senior_executive" ? "1 of 2" : "2 — Final"}. ${role !== "admin_senior_executive" ? "Leave balance will be deducted automatically." : "Branch Manager will complete final approval."}`}
